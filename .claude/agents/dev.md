...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `mkdocs build --strict`
  - Validate/verify/build command: `mkdocs build --strict`
...
- Run with `mkdocs build --strict` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `mkdocs build --strict` (or the
...
- `mkdocs build --strict` (or resolved validator) and the full suite pass ✓
